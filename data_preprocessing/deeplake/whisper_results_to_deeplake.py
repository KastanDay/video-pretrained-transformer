...
DATASET_PATH = f'/mnt/storage_ssd/dec_26_whisper_results_{BATCH_NAME}'
...
def main():
  ...
  if False and os.path.exists(DATASET_PATH):
    ...
    print(ds.summary())
    all_stems = ds.video_filename.data()['value']
  ...
  try:
    with ds:
      file_to_deeplake(stem_to_filepath_dict=stem_to_filepath_dict).eval(novel_filepaths,
                                                                         ds,
                                                                         scheduler='ray',
                                                                         num_workers=12)
...
def file_to_deeplake(clip_npz_path, sample_out, stem_to_filepath_dict):
  ...
  for segment_index in range(np_loaded['arr_0'].item()['total_segments']):
    try:
      ...
      sample_out.segment_metadata.append(json.dumps(local_segment_metadata))
