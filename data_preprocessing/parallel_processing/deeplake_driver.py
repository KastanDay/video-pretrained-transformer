...
class DeeplakeManager():
  ...
  def _clip_encode_results_to_deeplake(self): # Pylance ignore reportGeneralTypeIssues
    '''
    ...
    try:
      with self.ds:
        while self.upload_queue.qsize() > 0:
          ...
          assert last_idx-first_idx == (len(results['frames'])), print(f"Length of frames {len(results['frames'])} must equal {last_idx}-{first_idx} = {last_idx - first_idx}")
          ...
          print(f"⬆️⏰ Time to upload one batch: {(time.monotonic() - start_time):.2f} seconds. Time per frame = {((time.monotonic() - start_time)/len(results['frames'])):.2f}")
  ...
  def _text_encode_results_to_deeplake(self):
    ...
    try:
      with self.ds:
        while self.upload_queue.qsize() > 0:
          print("Upload queue size:", self.upload_queue.qsize())
          ...
          first_idx, last_idx = input_dict['db_indexes'][0], input_dict['db_indexes'][-1]
          last_idx = last_idx + 1 # Is this right? 
          assert check_continuity(input_dict['db_indexes']), print("db_inxexes must be continuous. This batch is not contiguous in Deeplake.")
          ...
          print(f"⬆️⬆️ Time to upload text-batch: {(time.monotonic() - start)/60:.2f} minutes. (time/segment): {((time.monotonic() - start)/len(caption_embed_dict_list)):.2f} sec")
          print(self.ds.summary())
    except Exception as e:
      print("-----------❌❌❌❌------------START OF ERROR-----------❌❌❌❌------------")
