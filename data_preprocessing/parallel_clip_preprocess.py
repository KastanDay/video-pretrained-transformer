...
# import OLD.OLD_CaptionPreprocessing as OLD_CaptionPreprocessing
import pathlib
import time
import json
import ray
...
HAVENT STARTED THIS YET --- all just starter code from Whisper parallel. 
...
FINAL_RESULTS_DESTINATION = f'/scratch/bbki/kastanday/whisper/{dir_name}_json'
...
NUM_THREADS = 70
NUM_CPUS = 1
GPU_PER_PROCESS = 1/12 # 1/12 is perfect balance on 4 gpus. Smaller demon = more spread across GPUs.
...
def parallel_caption_extraction(file_batch, itr):
    ...
    for index, file in enumerate(file_batch):
        ...
        try:
            ...
            # MAIN: run whisper
            process = OLD_CaptionPreprocessing.CaptionPreprocessing()
...
def rsync_inputs_to_workers():
    ...
    if not os.path.exists("/tmp/whisper_latest.sif"):
        print("Copying whisper.sif to /tmp")
        cp = ['cp', '-r', "/scratch/bbki/kastanday/whisper/whisper_latest.sif", '/tmp']
