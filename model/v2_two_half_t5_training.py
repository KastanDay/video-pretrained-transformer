    ...
    for scene_seg_list, clip_npz_path in tqdm.tqdm(zip(scene_reader, glob.glob(os.path.join(REMOTE_CLIP_DIR, '*'), recursive = True))):
        ...
        for segment_index in range(np_loaded['arr_0'].item()['total_segments']):
            try:
                ...
                # Split the list of words into halves using islice()
                scene_caption = scene_caption.split(" ") # to list of words
                first_half_of_whisper_caption  = " ".join(list(itertools.islice(scene_caption, 0, len(scene_caption)//2)))
                second_half_of_whisper_caption = " ".join(list(itertools.islice(scene_caption, len(scene_caption)//2, None)))
                first_half_of_whisper_caption_tokenized = clip.tokenize(first_half_of_whisper_caption).to(device)
                
                with torch.inference_mode(): # even faster than no_grad()
                    scene_embedding = clip_instance.encode_text(first_half_of_whisper_caption_tokenized)
                ...
                # print("Input shapes:", scene_embedding, caption_embedding, frame_embedding) # check shapes
                ''' Forward pass '''
                labels = t5_tokenizer(second_half_of_whisper_caption, return_tensors="pt").input_ids.to(device)
