...
if os.path.exists(MODEL_SAVE_PATH):
  ...
  t5 = T5ForConditionalGeneration.from_pretrained(f"{BASE_DIR}/MODEL_CHECKPOINTS/yt_pretrain_adamW_iter99000",
...
def my_dataloader_batching_trasnform(segment):
  clip_pooled_embedding = segment.clip_pooled_embedding.numpy()
  if clip_pooled_embedding.any():
    # clip_pooled_embedding = torch.from_numpy(segment.clip_pooled_embedding.numpy()).to(device)
    clip_last_hidden_states = torch.from_numpy(segment.clip_last_hidden_states.numpy()).to(device)
    caption_embedding = torch.from_numpy(segment.caption_embedding.numpy()).to(device)
    whisper_text_captions = segment.caption.data()['value']

    ## TODO: have to re-write this for batched dataloader...
    ## TODO: I should put these in the "transform" dataloader function
    ## That I way I pass everything into a Mosaic trainer.

    # split caption embedding into two halves
    s = caption_embedding.shape[0]
    s_half = s // 2
    caption_embedding_first_half = caption_embedding[:s_half]
    full_caption_tokenized = t5_tokenizer(whisper_text_captions, return_tensors="pt").input_ids.to(device)
    ...
    input_embeds_arr[0][1:578] = clip_last_hidden_states  # shape (577, 1024)
    input_embeds_arr[0][579:579 + s_half] = caption_embedding_first_half

    attn_mask_arr[0][0:579 + s_half] = 1
    # print("Total inputs", 579 + s_half)
    assert 579 + s_half <= 1024, print("Too many inputs")
    
    return input_embeds_arr, attn_mask_arr, labels
    
'''
https://docs.deeplake.ai/en/latest/deeplake.core.dataset.html#deeplake.core.dataset.Dataset.pytorch
...
dataloader = ds.pytorch(transform= num_workers=2, batch_size=4, pin_memory=False, shuffle=False, drop_last=False)

# iterate over segments
print("about to iterate over segments")
for i, segment in tqdm(enumerate(ds),
                       desc='training',
                       total=ds.max_len,
