...
class VPT_model(ComposerModel):
  ...
  def forward(self, batch):
    ...
    # removing attention mask
    return self.model.forward(inputs_embeds=c,  labels=l)
  ...
  def update_metric(self, outputs: Any, batch: Any, metric: Metric) -> None:
    ...
    if isinstance(metric, LanguageCrossEntropy):
      ...
      # set
      ground_truth_matrix = torch.zeros(self.vocab_size)
      yes_no_label = batch['label'].reshape(1, 2)  # tensor of [[105, 1]] due to stop token.
      ...
      if no_token_index in yes_no_label:
        ground_truth_matrix[no_token_index] = 1
      elif yes_no_label in yes_no_label:
        ground_truth_matrix[yes_token_index] = 1
      else:
  ...
  def vpt_transform_dataset_to_batch(self, deeplake_batch):
    ...
    print(deeplake_batch['label'][0])
    tokenizer = AutoTokenizer.from_pretrained(self.huggingface_model_name, return_special_tokens_mask=True)
    # Removing padding and truncation
    label_tokenized = tokenizer(deeplake_batch['label'][0], return_tensors="pt").input_ids
